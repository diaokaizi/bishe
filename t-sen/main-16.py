...
y_test['attack'] = y_test.apply(lambda row: "normal" if row.iloc[:5].sum() == 0 else row.iloc[-1], axis=1)
y_test['label'] = y_test.apply(lambda row: 0 if row.iloc[:5].sum() == 0 else 1, axis=1)
