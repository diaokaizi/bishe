...
from model_16 import Generator, Discriminator, Encoder
from tools import SimpleDataset, load_UGR16_16, load_UGR16, NormalizeTransform
import numpy as np
import pandas as pd
...
def main(opt):
    ...
    (x_train, y_train), (x_test, y_test) = load_UGR16_16()
    (x_train_bad, y_train), (x_test_bad, y_test) = load_UGR16()
    x_train = torch.cat([x_train, x_train_bad[:, :20]], dim=1)
    x_test = torch.cat([x_test, x_test_bad[:, :20]], dim=1)
